...
from transformers import BertModel, BertTokenizerFast, RobertaModel, RobertaTokenizerFast, DebertaTokenizerFast, DebertaModel
...
class MultiInferBert(torch.nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        if "roberta" in args.bert_model_path:
            self.bert = RobertaModel.from_pretrained(args.bert_model_path, return_dict=False)
            self.tokenizer = RobertaTokenizerFast.from_pretrained(args.bert_tokenizer)
        elif "deberta" in args.bert_model_path:
            self.bert = DebertaModel.from_pretrained(args.bert_model_path, return_dict=False)
            self.tokenizer = DebertaTokenizerFast.from_pretrained(args.bert_tokenizer)
        else:
            self.bert = BertModel.from_pretrained(args.bert_model_path, return_dict=False)
            self.tokenizer = BertTokenizerFast.from_pretrained(args.bert_tokenizer)
